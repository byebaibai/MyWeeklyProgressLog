...
for ri = 1:size(timewin2plot, 1)
    for ci = 1:size(timewin2plot, 2)
        [junk, timewin2plotidx(ri, ci)] = min(abs(EEG.times - timewin2plot(ri, ci)));
    end
end
...
% FFT of data
fft_total = fft(reshape(EEG.data, EEG.nbchan, EEG.pnts * EEG.trials), n_conv(1), 2);
fft_induced = fft(reshape(induced_EEG, EEG.nbchan, EEG.pnts * EEG.trials), n_conv(2),2); % non-phased-locked
fft_evoked = fft(erp, n_conv(3), 2); % phased-locked

fft_EEG{1} = fft_total;
fft_EEG{2} = fft_induced;
fft_EEG{3} = fft_evoked;

tf_res = zeros(3, length(freqs2plot), EEG.nbchan, EEG.pnts);

for fi = 1:length(freqs2plot)
    freq2plot = freqs2plot(fi);

    wavelet = exp(2 * 1i * freq2plot .* times) .* exp(-times.^2 ./ (2 * (4/(2 * pi * freq2plot))^2)) / freq2plot;
    for si=1:3
...
for fi = 1:length(freqs2plot)
    for ti = 1:length(timewin2plot)
        for li = 1:3
            data2plot = squeeze(mean(tf_res(li, fi, :, timewin2plotidx(ti, 1):timewin2plotidx(ti, 2)), 4));
            ...
            topoplot(data2plot, EEG.chanlocs, 'style', 'map', 'electrodes', 'off');
